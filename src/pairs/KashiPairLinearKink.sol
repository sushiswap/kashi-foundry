...
contract KashiPairLinearKink is ERC20, BoringOwnable, IMasterContract {
    ...
    KashiPairLinearKink public immutable masterContract;
    ...
    function name() external view returns (string memory) {
        return string(abi.encodePacked("Kashi Medium Risk 1.5 ", collateral.safeName(), "/", asset.safeName(), "-", oracle.name(oracleData)));
    }
    ...
    constructor(IBentoBoxV1 bentoBox_) {
        bentoBox = bentoBox_;
        masterContract = this;
    }
    ...
    function init(bytes calldata data) public payable override {
        ...
        require(bentoBox.balanceOf(address(asset), 0x000000000000000000000000000000000000dEaD) >= 1, "KashiPair: dead balance less");
    ...
    function accrue() public {
        ...
        _totalBorrow.elastic = uint128(_totalBorrow.elastic.add(extraAmount.to128()));
        uint256 fullAssetAmount = bentoBox.toAmount(address(asset), _totalAsset.elastic, false).add(_totalBorrow.elastic);
        ...
        _accrueInfo.feesEarnedFraction = uint128(_accrueInfo.feesEarnedFraction.add(feeFraction.to128()));
        totalAsset.base = uint128(_totalAsset.base.add(feeFraction.to128()));
        ...
        _accrueInfo.interestPerSecond = _computeInterestRate(utilization);
    ...
    uint64 private constant BASE_INTEREST_PER_SECOND = 0;
    uint64 private constant INTEREST_RATE_SLOPE1 = 1585489600;
    uint64 private constant INTEREST_RATE_SLOPE2 = 0;
    ...
    function _addTokens(
        ...
        if (skim) {
            require(share <= bentoBox.balanceOf(address(token), address(this)).sub(total), "KashiPair: Skim too much");
        } else {
            bentoBox.transfer(address(token), msg.sender, address(this), share);
        }
    }
    ...
    function _removeCollateral(address to, uint256 share) internal {
        ...
        bentoBox.transfer(address(collateral), address(this), to, share);
    }
    ...
    function _addAsset(
        ...
        uint256 allShare = _totalAsset.elastic + bentoBox.toShare(address(asset), totalBorrow.elastic, true);
    ...
    function _removeAsset(address to, uint256 fraction) internal returns (uint256 share) {
        ...
        _totalAsset.elastic = uint128(_totalAsset.elastic.sub(share.to128()));
        _totalAsset.base = uint128(_totalAsset.base.sub(fraction.to128()));
        require(_totalAsset.base >= 1000, "Kashi: below minimum");
        totalAsset = _totalAsset;
        emit LogRemoveAsset(msg.sender, to, share, fraction);
        bentoBox.transfer(address(asset), address(this), to, share);
    }
    ...
    function _borrow(address to, uint256 amount) internal returns (uint256 part, uint256 share) {
        ...
        share = bentoBox.toShare(address(asset), amount, false);
        Rebase memory _totalAsset = totalAsset;
        require(_totalAsset.base >= 1000, "Kashi: below minimum");
        _totalAsset.elastic = uint128(_totalAsset.elastic.sub(share.to128()));
        totalAsset = _totalAsset;
    ...
    function _repay(
        ...
        uint256 share = bentoBox.toShare(address(asset), amount, true);
        uint128 totalShare = totalAsset.elastic;
        _addTokens(asset, share, uint256(totalShare), skim);
        totalAsset.elastic = uint128(totalShare.add(share.to128()));
    ...
    function _bentoDeposit(
        ...
        return bentoBox.deposit{value: value}(address(token), msg.sender, to, uint256(amount), uint256(share));
    }
    ...
    function _bentoWithdraw(
        ...
        (IERC20 token, address to, int256 amount, int256 share) = abi.decode(data, (IERC20, address, int256, int256));
        return bentoBox.withdraw(address(token), msg.sender, to, _num(amount, value1, value2), _num(share, value1, value2));
    }
    ...
    function cook(
        ...
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            if (action == ACTION_ADD_COLLATERAL) {
                ...
                bentoBox.transfer(address(token), msg.sender, to, _num(share, value1, value2));
            } else if (action == ACTION_BENTO_TRANSFER_MULTIPLE) {
                (IERC20 token, address[] memory tos, uint256[] memory shares) = abi.decode(datas[i], (IERC20, address[], uint256[]));
                bentoBox.transferMultiple(address(token), msg.sender, tos, shares);
                ...
                value1 = bentoBox.toShare(address(asset), totalBorrow.toElastic(_num(part, value1, value2), true), true);
    ...
    function liquidate(
        ...
        Rebase memory _totalBorrow = totalBorrow;
        Rebase memory bentoBoxTotals = bentoBox.totals(address(collateral));
        ...
        _totalBorrow.elastic = uint128(_totalBorrow.elastic.sub(allBorrowAmount.to128()));
        _totalBorrow.base = uint128(_totalBorrow.base.sub(allBorrowPart.to128()));
        totalBorrow = _totalBorrow;
        totalCollateralShare = totalCollateralShare.sub(allCollateralShare);

        uint256 allBorrowShare = bentoBox.toShare(address(asset), allBorrowAmount, true);
        ...
        if (!open) {
            ...
            bentoBox.transfer(address(collateral), address(this), address(swapper), allCollateralShare);
            swapper.swap(address(collateral), address(asset), address(this), allBorrowShare, allCollateralShare);

            uint256 returnedShare = bentoBox.balanceOf(address(asset), address(this)).sub(uint256(totalAsset.elastic));
            ...
            bentoBox.transfer(address(asset), address(this), masterContract.feeTo(), feeShare);
            totalAsset.elastic = uint128(totalAsset.elastic.add(returnedShare.sub(feeShare).to128()));
            emit LogAddAsset(address(swapper), address(this), extraShare.sub(feeShare), 0);
        } else {
            // Swap using a swapper freely chosen by the caller
            // Open (flash) liquidation: get proceeds first and provide the borrow after
            bentoBox.transfer(address(collateral), address(this), swapper == ISwapper(address(0)) ? to : address(swapper), allCollateralShare);
            if (swapper != ISwapper(address(0))) {
                swapper.swap(address(collateral), address(asset), msg.sender, allBorrowShare, allCollateralShare);
            }

            bentoBox.transfer(address(asset), msg.sender, address(this), allBorrowShare);
            totalAsset.elastic = uint128(totalAsset.elastic.add(allBorrowShare.to128()));
        }
    }
